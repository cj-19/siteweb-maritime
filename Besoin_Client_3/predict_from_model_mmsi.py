...
if len(sys.argv) != 2:
    print("Utilisation : python monscript.py <MMSI>")
    sys.exit()

try:
    mmsi = int(sys.argv[1])
except ValueError:
    print("Erreur : Le MMSI doit être un entier.")
    sys.exit()

# === 1. Paramètres ===
base_dir = "/var/www/etu0203/projet_ais/Besoin_Client_3"
...
# === 3. Vérification du MMSI disponible ===
...
if mmsi not in available_mmsi:
    print(f"Modèle introuvable pour le MMSI {mmsi}.")
    print("Modèles disponibles :", available_mmsi)
    sys.exit()

# === 4. Chargement du modèle et des encoders ===
...
# === 7. Création de timedelta et recherche d’une fenêtre valide ===
